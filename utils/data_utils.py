...
def build_outputs(hyperparams, num_jobs=8):
    data_dir = hyperparams.input_path.rstrip("/")
    anno_paths = glob.glob(f"{data_dir}/train/annotations/*.json")
    annotations = Parallel(n_jobs=num_jobs, verbose=1, backend="threading")(
...
def process_annotations(hyperparams, num_jobs=8):
    data_dir = hyperparams.input_path.rstrip("/")
    anno_paths = glob.glob(f"{data_dir}/annotations/*.json")
    annotations = Parallel(n_jobs=num_jobs, verbose=1, backend="threading")(
